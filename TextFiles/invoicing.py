...
def next_invoice_number(invoice_number: str) -> str:
    ...
    if year == invoice_year:
        ...
        invoice_year = year
        number = 1
    new_invoice_number = f'{invoice_year}-{number:04d}'  # used a format specifier to get the number correctly 0001
...
def record_invoice(invoice_file: TextIO,
    ...
    for line in invoice_file:
        ...
        last_row = line
    if last_row:
        # invoice_number, _, _ = last_row.split('\t')  # the _ is used to signalise that we won't be using that data
        invoice_number = last_row.split('\t')[0]
        new_invoice_number = next_invoice_number(invoice_number)
    else:
        # if the file is empty, well start the numbering from 1
        year = get_year()
